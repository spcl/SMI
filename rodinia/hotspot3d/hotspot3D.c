...
void usage(int argc, char** argv) {
  ...
  fprintf(stderr,
          "\t<powerFile>  - name of the file containing the initial power "
          "values of each cell\n");
  fprintf(stderr,
  ...
  fprintf(stderr,
          "\tNote: If input file names are not supplied, input is generated "
          "randomly.\n");
  fprintf(stderr,
...
int main(int argc, char** argv) {
  ...
  if (argc == 7) {
    ...
    if (version >= 7) {
      for (i = PAD; i < size; i++) {
        pIn[i] =
            (float)rand() / (float)(RAND_MAX);  // random number between 0 and 1
        tIn[i] = 300 + (float)rand() /
                           (float)(RAND_MAX /
                                   100);  // random number between 300 and 400
      }
    } else {
      for (i = 0; i < size; i++) {
        pIn[i] =
  ...
  if (version >= 7) {
    ...
    int last_col =
        ((numCols % comp_bsize_x == 0)
             ? numCols + 0
             : numCols + comp_bsize_x - numCols % comp_bsize_x) -
        comp_bsize_x;  // exit variable is first compared, then incremented
    int last_row =
        ((numRows % comp_bsize_y == 0)
             ? numRows + 0
             : numRows + comp_bsize_y - numRows % comp_bsize_y) -
        comp_bsize_y;  // exit variable is first compared, then incremented
    int col_blocks = (last_col / comp_bsize_x) + 1;
    int row_blocks = (last_row / comp_bsize_y) + 1;
    int comp_exit =
        ((BLOCK_X * col_blocks * BLOCK_Y * row_blocks * (layers + RAD)) /
         SSIZE);  // exit variable is first incremented, then compared
    int mem_exit = (BLOCK_X * col_blocks * BLOCK_Y * row_blocks * layers) /
                   SSIZE;  // exit variable is first incremented, then compared
    ...
    int row_blocks = (last_row / comp_bsize_y) + 1;
    int comp_exit =
        ((BLOCK_X * col_blocks * BLOCK_Y * row_blocks * (layers + 1)) /
         SSIZE);  // exit variable is first incremented, then compared
  ...
  printf("Throughput is %0.3lf GBps.\n",
         (3 * numCols * numRows * layers * sizeof(float) * iterations) /
             (1000000000.0 * computeTime / 1000.0));

#ifdef VERIFY
  float* answer = (float*)calloc(size, sizeof(float));
  if (version >= 7) {
    computeTempCPU(pIn + PAD, tempCopy + PAD, answer + PAD, numCols, numRows,
                   layers, Cap, Rx, Ry, Rz, dt, amb_temp, iterations);
  } else {
