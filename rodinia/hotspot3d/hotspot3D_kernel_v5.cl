...
#define IN_SR_BASE \
  2 * BLOCK_X*     \
          BLOCK_Y  // this shows the point to write into the shift register; one
                   // row for top neighbors and one row for center
#define IN_SR_SIZE \
  IN_SR_BASE + SSIZE  // SSIZE indexes are enough for the bottommost row
...
#define SR_OFF_A \
  SR_OFF_C + BLOCK_X* BLOCK_Y  // above  (-z direction), this is intentionally
                               // written like this to follow the behavior of
                               // the baseline implementation
#define SR_OFF_B \
  SR_OFF_C - BLOCK_X* BLOCK_Y  // below  (+z direction), this is intentionally
                               // written like this to follow the behavior of
                               // the baseline implementation

__attribute__((max_global_work_dim(0))) __kernel void hotspotOpt1(
    ...
    const float ce, const float cw, const float cn, const float cs,
    const float ct, const float cb, const float cc,
    ...
    const int comp_exit)  // exit condition for compute loop
{
  ...
  while (index != comp_exit) {
    ...
    for (int i = 0; i < SSIZE; i++) {
      ...
      int comp_offset_z = z - 1;  // global z position, will be out-of-bound for
                                  // first and last iterations
      int comp_offset =
          gx + gy * nx +
          comp_offset_z * nx *
              ny;  // index offset for computation, reading from power buffer,
                   // and writing back to destination
      int read_offset =
          comp_offset + nx * ny;  // index offset to read from memory
      int real_block_x =
          x + i - 1;             // local x position in block, adjusted for halo
      ...
      if (real_x >= 0 && gy >= 0 && real_x < nx && gy < ny) {
        ...
        if (comp_offset_z != nz - 1) {
          in_sr[IN_SR_BASE + i] =
              src[read_offset + i];  // read new temperature values from memory
        }

        // there is nothing to compute on the first out-of-bound plane and the
        // halos conditions on x dimension have been avoided since they only
        // save a few memory accesses and could cause access splitting
        if (comp_offset_z >= 0 && real_block_y >= 0 &&
            real_block_y < BLOCK_Y - 2) {
          local_pwr =
              pwr_in[comp_offset + i];  // read new power values from memory
        }
      }
      ...
      // below neighbor, the condition is intentionally written like this to
      // follow the behavior of the baseline implementation
      ...
      // above neighbor, the condition is intentionally written like this to
      // follow the behavior of the baseline implementation
      ...
      // all axes
      if (comp_offset_z >= 0 && real_block_x >= 0 && real_block_y >= 0 &&
          real_block_x < BLOCK_X - 2 * 1 && real_block_y < BLOCK_Y - 2 * 1 &&
          real_x < nx && gy < ny) {
        dst[comp_offset + i] = center * cc + north * cn + south * cs +
                               east * ce + west * cw + above * ct + below * cb +
                               local_pwr * sdc + AMB_TEMP * ct;
      }
    }

    // equivalent to x = (x + SSIZE) % BLOCK_X
    x = (x + SSIZE) & (BLOCK_X - 1);  // move one chunk forward and reset to
                                      // zero if end of row was reached
    ...
    if (x == 0)  // if one row finished
    {
      ...
      {
        if (z == nz)  // if on last compute plane (compute traverses one more
                      // plane than memory read/write)
        {
          ...
          if (bx == last_col)  // border of extended input grid in x direction
          {
            ...
            by +=
                BLOCK_Y -
                2;  // go to next block in y direction, account for overlapping
          } else {
            bx +=
                BLOCK_X -
                2;  // go to next block in x direction, account for overlapping
          }
        } else {
          z++;  // go to next plane
        }
