...
__attribute__((max_global_work_dim(0))) __kernel void hotspot(
    ...
    const float Rx_1, const float Ry_1, const float Rz_1, const int comp_exit, const int t) {
  float in_sr[IN_SR_SIZE];  // shift register for spatial blocking

  const int db_read_offset = (t % 2 == 0) ? 0 : grid_cols * grid_rows;
  const int db_write_offset = (t % 2 == 0) ? grid_cols * grid_rows : 0;
  ...
  while (index != comp_exit) {
    ...
    for (int i = 0; i < SSIZE; i++) {
      ...
      int comp_offset_y = y - 1;  // global y position, will be out-of-bound for
                                  // first and last iterations
      int comp_offset =
          gx + comp_offset_y *
                   grid_cols;  // index offset for computation, reading from
      ...
      if (real_x >= 0 &&
          real_x < grid_cols)  // avoid out-of-bound indexes in x direction
      {
        // nothing to read on last row
        if (comp_offset_y != grid_rows - 1) {
          in_sr[IN_SR_BASE + i] =
              src[db_read_offset + read_offset +
                  i];  // read new temperature values from memory
        }
      ...
      // top neighbor
      if (comp_offset_y == grid_rows - 1)  // topmost row, deliberately written
                                           // like this to align with v1 kernel
      {
      ...
      // bottom neighbor
      if (comp_offset_y == 0)  // bottommost row, deliberately written like this
      ...
      // all axes
      if (comp_offset_y >= 0 && real_block_x >= 0 && real_block_x < BSIZE - 2 &&
          real_x < grid_cols) {
        dst[db_write_offset + comp_offset + i] = center + delta;
      }

      // if (ID == 1 && comp_offset_y >=0 && comp_offset_y < grid_rows && real_x
      // >=0 && real_x < grid_cols) printf("ID: %d, row: %04d, col: %04d, center:
      // %f, top: %f, bottom: %f, left: %f, right: %f, pow: %f, out: %f\n", ID,
      // comp_offset_y, real_x, center, north, south, west, east, local_pwr,
      // out.data[i]);
    }

    // equivalent to x = (x + SSIZE) % BSIZE
    x = (x + SSIZE) & (BSIZE - 1);  // move one chunk forward and reset to zero
                                    // if end of row was reached
