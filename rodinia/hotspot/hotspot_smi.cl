...
#define IN_SR_BASE \
  2 * BSIZE  // this shows the point to write into the shift register; one row
             // for top neighbors and one row for center
#define IN_SR_SIZE \
...
__attribute__((max_global_work_dim(0))) __kernel void hotspot(
    ...
    const float Rx_1, const float Ry_1, const float Rz_1,
  ...
  for (int t = 0; t < iterations; ++t) {
    ...
    // Double buffering
    const int smi_offset = (smi_rank > 0 && smi_size > 1) ? grid_cols : 0;
    const int db_read_offset =
        smi_offset + ((t % 2 == 0) ? 0 : grid_rows * grid_cols);
    const int db_write_offset =
        smi_offset + ((t % 2 == 0) ? grid_rows * grid_cols : 0);
    ...
    while (index != comp_exit) {
      ...
      for (int i = 0; i < SSIZE; i++) {
        ...
        // global y position, will be out-of-bound
        // for first and last iterations
        int comp_offset_y = y - 1;
        // index offset for computation, reading from
        // power buffer, and writing back to destination
        ...
        if (real_x >= 0 && real_x < grid_cols) {
          // nothing to read on last row
          if (comp_offset_y != grid_rows - 1) {
            // read new temperature values from memory
            in_sr[IN_SR_BASE + i] = src[db_read_offset + read_offset + i];
          }
        ...
        // top neighbor
        // If we're not the top SMI rank, this is still in bounds
        if (comp_offset_y == grid_rows - 1 &&
            (smi_size <= 1 || smi_rank != 0)) {
        ...
        // bottom neighbor
        // If we're not the bottom SMI rank, this is still in bounds
        if (comp_offset_y == 0 && (smi_rank < smi_size - 1 || smi_size <= 1)) {
          south = center;  // fall back on center
        } else {
          south = in_sr[SR_OFF_S + i];  // read from shift register
        }

        float v = local_pwr + (north + south - 2.0f * center) * Ry_1 +
        ...
        // all axes
        if (comp_offset_y >= 0 && real_block_x >= 0 &&
            real_block_x < BSIZE - 2 && real_x < grid_cols) {
          dst[db_write_offset + comp_offset + i] = center + delta;
        }

        // if (ID == 1 && comp_offset_y >=0 && comp_offset_y < grid_rows &&
        // real_x
        // >=0 && real_x < grid_cols) printf("ID: %d, row: %04d, col: %04d,
        // center: %f, top: %f, bottom: %f, left: %f, right: %f, pow: %f, out:
        // %f\n", ID, comp_offset_y, real_x, center, north, south, west, east,
        // local_pwr, out.data[i]);
      }

      // equivalent to x = (x + SSIZE) % BSIZE
      x = (x + SSIZE) & (BSIZE - 1);  // move one chunk forward and reset to
                                      // zero if end of row was reached

      if (x == 0)  // if one row finished
      {
        if (y == grid_rows)  // if on last row (compute traverses one more plane
