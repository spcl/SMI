...
int compute_tran_temp(cl_mem MatrixPower, cl_mem MatrixTemp[2], int col,
  ...
  if (is_ndrange_kernel(version_number)) {
    ...
    if (version_number == 2) {
      ...
      CL_SAFE_CALL(
          clSetKernelArg(kernel, 8, sizeof(float), (void *)&step_div_Cap));
      ...
      CL_SAFE_CALL(clSetKernelArg(kernel, 5, sizeof(int), (void *)&row));
      CL_SAFE_CALL(
          clSetKernelArg(kernel, 6, sizeof(int), (void *)&pyramid_height));
      CL_SAFE_CALL(
          clSetKernelArg(kernel, 7, sizeof(float), (void *)&step_div_Cap));
    ...
    for (t = 0; t < total_iterations; t += pyramid_height) {
      ...
      if (version_number == 2) {
        CL_SAFE_CALL(
            clSetKernelArg(kernel, 12, sizeof(int), (void *)&small_block_rows));
        CL_SAFE_CALL(
            clSetKernelArg(kernel, 13, sizeof(int), (void *)&small_block_cols));
      } else if (version_number == 4) {
        CL_SAFE_CALL(
            clSetKernelArg(kernel, 11, sizeof(int), (void *)&small_block_rows));
        CL_SAFE_CALL(
            clSetKernelArg(kernel, 12, sizeof(int), (void *)&small_block_cols));
      }
    ...
    if (version_number >= 7) {
      ...
      CL_SAFE_CALL(
          clSetKernelArg(ReadKernel, 8, sizeof(float), (void *)&comp_exit));
      CL_SAFE_CALL(
          clSetKernelArg(ReadKernel, 9, sizeof(float), (void *)&mem_exit));
      ...
      CL_SAFE_CALL(
          clSetKernelArg(WriteKernel, 3, sizeof(float), (void *)&mem_exit));
    ...
    int t;
    if (version_number >= 7) {
      for (t = 0; t < total_iterations;
           t = t +
               TIME)  // in this version two iterations are launched
                      // simultaneously, hence total iteration number is halved
      {
        CL_SAFE_CALL(clSetKernelArg(ReadKernel, 0, sizeof(cl_mem),
                                    (void *)&MatrixTemp[src]));
        CL_SAFE_CALL(clSetKernelArg(WriteKernel, 0, sizeof(cl_mem),
                                    (void *)&MatrixTemp[dst]));

        // the following variable is used to choose which time step should send
        // its output to the write kernel unless there are less iterations left
        // than there are parallel time steps, the last time step will do so
        int rem_iter =
            (total_iterations - t > TIME) ? TIME : total_iterations - t;
        CL_SAFE_CALL(
            clSetKernelArg(ReadKernel, 10, sizeof(float), (void *)&rem_iter));
...
#ifdef EMULATOR
        CL_SAFE_CALL(
            clEnqueueTask(command_queue3, ComputeKernel, 0, NULL, NULL));
#endif

        clFinish(command_queue2);  // this is necessary since the two kernels
                                   // are running in two different queue and a
                                   // new iteration should not start before the
...
int main(int argc, char **argv) {
...
#ifdef EMULATOR
  if (version_number >= 7) {
    command_queue3 = clCreateCommandQueue(context, device,
                                          CL_QUEUE_PROFILING_ENABLE, &error);
    if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
  }
#endif
  ...
  size = (version_number < 7) ? grid_rows * grid_cols
                              : grid_rows * grid_cols + PAD;
  ...
  cl_program program = clCreateProgramWithSource(
      context, 1, (const char **)&source, NULL, &error);
#else
  cl_program program =
  ...
  if (version_number == 4) {
    sprintf(clOptions + strlen(clOptions),
            " -DBLOCK_X=%d -DBLOCK_Y=%d -DSSIZE=%d", block_size_x, block_size_y,
            SSIZE);
  } else {
    sprintf(clOptions + strlen(clOptions), " -DBSIZE=%d -DSSIZE=%d",
            block_size_x, SSIZE);
  }
#endif
