...
int main(int argc, char *argv[]) {
  ...
  cl_kernel diagonal = NULL, perimeter = NULL,
            internal = NULL;  // for NDRange kernels
  ...
  // the function.
  while ((opt = getopt_long(argc, argv, "::vs:i:b:", long_options,
                            &option_index)) != -1) {
    switch (opt) {
      case 'i':
        input_file = optarg;
  ...
  CL_SAFE_CALL(clEnqueueWriteBuffer(cmd_queue, d_m, 1, 0,
                                    matrix_dim * matrix_dim * sizeof(float), m,
                                    0, 0, 0));

  // create kernels
  diagonal = clCreateKernel(prog, "lud_diagonal", &err);
  if (err != CL_SUCCESS) {
    printf("ERROR: clCreateKernel(diagonal) 0 => %d\n", err);
    return -1;
  }
  perimeter = clCreateKernel(prog, "lud_perimeter", &err);
  if (err != CL_SUCCESS) {
    printf("ERROR: clCreateKernel(perimeter) 0 => %d\n", err);
    return -1;
  }
  internal = clCreateKernel(prog, "lud_internal", &err);
  if (err != CL_SUCCESS) {
    printf("ERROR: clCreateKernel(internal) 0 => %d\n", err);
    return -1;
  }
  ...
  CL_SAFE_CALL(
      clSetKernelArg(diagonal, 1, sizeof(cl_int), (void *)&matrix_dim));

  CL_SAFE_CALL(clSetKernelArg(perimeter, 0, sizeof(void *), (void *)&d_m));
  CL_SAFE_CALL(
      clSetKernelArg(perimeter, 1, sizeof(cl_int), (void *)&matrix_dim));

  CL_SAFE_CALL(clSetKernelArg(internal, 0, sizeof(void *), (void *)&d_m));
  CL_SAFE_CALL(
      clSetKernelArg(internal, 1, sizeof(cl_int), (void *)&matrix_dim));
  ...
  for (int i = 0; i < matrix_dim - BSIZE; i += BSIZE) {
    if (version == 4) {
      ...
      CL_SAFE_CALL(
          clSetKernelArg(diagonal, 2, sizeof(cl_int), (void *)&offset));
      CL_SAFE_CALL(
          clSetKernelArg(perimeter, 2, sizeof(cl_int), (void *)&offset));
      CL_SAFE_CALL(
          clSetKernelArg(internal, 2, sizeof(cl_int), (void *)&offset));
    } else {
    ...
    if (is_ndrange_kernel(version)) {
      ...
      CL_SAFE_CALL(clEnqueueNDRangeKernel(cmd_queue, diagonal, 2, NULL,
      ...
      CL_SAFE_CALL(clEnqueueNDRangeKernel(cmd_queue, perimeter, 2, NULL,
      ...
      CL_SAFE_CALL(clEnqueueNDRangeKernel(cmd_queue, internal, 2, NULL,
      ...
      if (version == 1) {
        ...
        for (int chunk_idx = 0; chunk_idx < chunk_num; chunk_idx++) {
          ...
          CL_SAFE_CALL(
              clSetKernelArg(perimeter, 3, sizeof(cl_int), (void *)&offset));
      ...
      if (version == 1) {
        ...
        for (int chunk_idx = 0; chunk_idx < chunk_num * chunk_num;
             chunk_idx++) {
          int i_global = i + BSIZE * (1 + chunk_idx / chunk_num);
          int j_global = i + BSIZE * (1 + chunk_idx % chunk_num);

          CL_SAFE_CALL(
              clSetKernelArg(internal, 3, sizeof(cl_int), (void *)&i_global));
          CL_SAFE_CALL(
              clSetKernelArg(internal, 4, sizeof(cl_int), (void *)&j_global));
  ...
  int offset = (version == 4)
                   ? (matrix_dim - BSIZE) * matrix_dim + (matrix_dim - BSIZE)
                   : matrix_dim - BSIZE;
  ...
  CL_SAFE_CALL(clEnqueueReadBuffer(cmd_queue, d_m, 1, 0,
                                   matrix_dim * matrix_dim * sizeof(float), m,
                                   0, 0, 0));
  clFinish(cmd_queue);
