...
#define IN_SR_BASE \
  BSIZE + 1  // this shows the point to write into the shift register; one row
             // to reach north neighbor and one extra cell for north_west
#define IN_SR_SIZE \
  IN_SR_BASE + SSIZE  // SSIZE extra indexes to support vectorization
...
#define HALO_SIZE \
  rem_rows  // the dependency pattern forms a cone with the same base size and
            // height

__attribute__((max_global_work_dim(0))) __kernel void dynproc_kernel(
    __global int* restrict wall, __global int* restrict src,
    __global int* restrict dst, int cols, int rem_rows, int starting_row,
    int comp_exit) {
  ...
  while (index != comp_exit) {
    ...
    for (int i = 0; i < SSIZE; i++) {
      ...
      int read_offset =
          real_x +
          (real_y - 1) * cols;  // index to read from memory, wall input starts
                                // from second row of the input grid
      int real_block_x =
          x + i - HALO_SIZE;  // local x position in block, adjusted for halo

      int north = 0x7ffffff, north_east = 0x7ffffff, north_west = 0x7ffffff,
          center = 0x7ffffff;  // initialize to biggest positive integer
      int min = 0;
      ...
      // all axes since the dependency pattern forms a cone, y gets involved in
      // this case
      if (real_block_x >= y - HALO_SIZE && real_block_x < BSIZE - 2 * y &&
          real_x >= 0 && real_x < cols) {
      ...
      // all axes since the dependency pattern forms a cone, y gets involved in
      // this case
      if (real_y >= 0 && real_block_x >= y - HALO_SIZE &&
          real_block_x < BSIZE - 2 * y && real_x >= 0 && real_x < cols) {
        shift_reg[IN_SR_BASE + i] = center + min;
        if (y == rem_rows) {
          dst[real_x] = shift_reg[IN_SR_BASE + i];
        }
        // if (y > 0)
        // printf("row: %04d, col: %04d, center: %d, N: %d, NE: %d, NW: %d, min:
        // %d, out: %d\n", real_y, real_x, center, north, north_east, north_west,
        // min, shift_reg[IN_SR_BASE + i]);
      }
    }

    x = (x + SSIZE) &
        (BSIZE - 1);  // move one chunk forward and reset to zero if end of row
                      // was reached, alternative for x = (x + SSIZE) % BSIZE

    if (x == 0)  // if one row finished
    {
      y++;  // go to next row

      if (y == rem_rows + 1)  // if on last row in block, traverses one more row
                              // than it should for write-back
      {
        y = 0;  // reset row number
        bx +=
            BSIZE - 2 * HALO_SIZE;  // go to next block, account for overlapping
      }
    }
  }
}
