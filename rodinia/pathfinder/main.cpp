...
int main(int argc, char** argv) {
  ...
  if (is_ndrange_kernel(version) || version == 5) {
    ...
    CL_SAFE_CALL(clEnqueueWriteBuffer(cl.command_queue, d_gpuWall, CL_TRUE, 0,
                                      sizeof(cl_int) * (size), data, 0, NULL,
                                      NULL));
    CL_SAFE_CALL(clEnqueueWriteBuffer(cl.command_queue, d_gpuResult[0], CL_TRUE,
                                      0, sizeof(cl_int) * cols, data, 0, NULL,
                                      NULL));
  ...
  if (is_ndrange_kernel(version)) {
    ...
    // Set fixed kernel arguments
    CL_SAFE_CALL(
        clSetKernelArg(cl.kernel(kn), 0, sizeof(cl_mem), (void*)&d_gpuWall));
    CL_SAFE_CALL(
        clSetKernelArg(cl.kernel(kn), 3, sizeof(cl_int), (void*)&cols));
    ...
    for (int t = 0; t < rows - 1; t++) {
      ...
      // Calculate changed kernel arguments...
      CL_SAFE_CALL(clSetKernelArg(cl.kernel(kn), 1, sizeof(cl_mem),
                                  (void*)&d_gpuResult[src]));
      CL_SAFE_CALL(clSetKernelArg(cl.kernel(kn), 2, sizeof(cl_mem),
                                  (void*)&d_gpuResult[dst]));
    ...
    for (int startStep = 0; startStep < rows - 1; startStep += pyramid_height) {
      ...
      int rem_rows =
          MIN(pyramid_height,
              rows - startStep - 1);  // either equal to pyramid_height or the
                                      // number of remaining iterations

      // Exit condition should be a multiple of comp_bsize
      int comp_bsize = BSIZE - 2 * rem_rows;
      int last_col = (cols % comp_bsize == 0)
                         ? cols + 0
                         : cols + comp_bsize - cols % comp_bsize;
      ...
      CL_SAFE_CALL(clSetKernelArg(cl.kernel(kn), 2, sizeof(cl_mem),
                                  (void*)&d_gpuResult[dst]));
      CL_SAFE_CALL(
          clSetKernelArg(cl.kernel(kn), 4, sizeof(cl_int), (void*)&rem_rows));
      CL_SAFE_CALL(
          clSetKernelArg(cl.kernel(kn), 5, sizeof(cl_int), (void*)&startStep));
      CL_SAFE_CALL(
          clSetKernelArg(cl.kernel(kn), 6, sizeof(cl_int), (void*)&comp_exit));
