...
static inline void compute_flux_contribution(
    ...
    FLOAT3 velocity, FLOAT3* restrict fc_momentum_x,
    FLOAT3* restrict fc_momentum_y, FLOAT3* restrict fc_momentum_z,
...
__attribute__((reqd_work_group_size(BSIZE, 1, 1))) __kernel void
compute_step_factor(__global float* restrict variables,
                    __global float* restrict areas,
                    __global float* restrict step_factors, int nelr) {
...
__attribute__((num_compute_units(CUSIZE))) __kernel void
compute_flux(__global int* restrict elements_surrounding_elements,
             __global float* restrict normals,
             __global volatile float* restrict variables,
  ...
  for (j = 0; j < NNB; j++) {
    ...
    if (nb >= 0)  // a legitimate neighbor
    {
      ...
      factor =
          -normal_len * smoothing_coefficient * (float)(0.5f) *
...
__attribute__((reqd_work_group_size(BSIZE, 1, 1))) __kernel void time_step(
    int j, int nelr, __global float* restrict old_variables,
    __global float* restrict variables, __global float* restrict step_factors,
    __global float* restrict fluxes) {
  // const int i = (blockDim.x*blockIdx.x + threadIdx.x);
  const int i = get_global_id(0);
  if (i >= nelr) return;
