...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < max_cols; i++) {
    for (int j = 1; j < max_rows; j++) {
      int ref_offset =
          (version_number == 5) ? i * num_cols + (j - 1) : i * num_cols + j;
  ...
  size_t local_work[3] = {(size_t)((workgroupsize > 0) ? workgroupsize : 1), 1,
                          1};
  size_t global_work[3] = {(size_t)nworkitems, 1,
...
#ifdef NO_INTERLEAVE
  cl_mem reference_d =
      clCreateBuffer(context, CL_MEM_READ_ONLY | CL_CHANNEL_1_INTELFPGA,
  ...
  if (version_number == 5) {
    CL_SAFE_CALL(clEnqueueWriteBuffer(cmd_queue, input_itemsets_d, 1, 0,
                                      num_cols * sizeof(int), buffer_h, 0, 0,
                                      0));
    CL_SAFE_CALL(clEnqueueWriteBuffer(cmd_queue, buffer_v_d, 1, 0,
                                      num_rows * sizeof(int), buffer_v, 0, 0,
                                      0));
  } else {
    CL_SAFE_CALL(clEnqueueWriteBuffer(cmd_queue, input_itemsets_d, 1, 0,
                                      data_size * sizeof(int), input_itemsets,
                                      0, 0, 0));
  }
  ...
  if (is_ndrange_kernel(version_number)) {
    ...
    int cols =
        num_cols - 1 + PAR;  // -1 since last column is invalid, +PAR to make
                             // sure all cells in the last chunk are processed
    ...
    CL_SAFE_CALL(
        clSetKernelArg(kernel1, 1, sizeof(void *), (void *)&input_itemsets_d));
    CL_SAFE_CALL(
        clSetKernelArg(kernel1, 2, sizeof(void *), (void *)&buffer_v_d));
    ...
    CL_SAFE_CALL(
        clSetKernelArg(kernel1, 5, sizeof(cl_int), (void *)&loop_exit));
  }
  ...
  if (is_ndrange_kernel(version_number)) {
    ...
    int last_diag = (num_diags % comp_bsize == 0)
                        ? num_diags
                        : num_diags + comp_bsize - (num_diags % comp_bsize);
    ...
    for (int bx = 0; bx < num_blocks; bx++) {
      ...
      CL_SAFE_CALL(
          clSetKernelArg(kernel1, 6, sizeof(cl_int), (void *)&block_offset));
  ...
  for (int i = 1; i < num_rows - 2; ++i) {
    for (int j = start_j; j < num_cols - 2; ++j) {
      fprintf(fout, "[%d, %d] = %d (ref: %d)\n", i - 1, j - start_j,
              output_itemsets[i * num_cols + j], reference[i * num_cols + j]);
    }
  }
  fclose(fout);
