...
__attribute__((max_global_work_dim(0))) __kernel void nw_kernel1(
    __global int* restrict reference, __global int* restrict data,
    __global int* restrict input_v,  // vertical input (first column)
    int dim, int penalty, int loop_exit, int block_offset) {
  int out_SR[PAR - 1][3];  // output shift register; 2 registers per parallel
                           // comp_col_offset is required, one for writing, and
                           // two for passing data to the following diagonal
                           // lines to handle the dependency
  int last_chunk_col_SR
      [BSIZE - (PAR - 1) +
       2];  // shift register for last comp_col_offset in parallel chunk to pass
            // data to next chunk, (PAR - 1) cells are always out of bound, one
            // extra cell is added for writing and one more for top-left
  int ref_SR[PAR]
            [PAR];  // shift registers to align reads from the reference buffer
  int data_h_SR[PAR][PAR];  // shift registers to align reading the first
                            // comp_row of data buffer
  ...
  while (loop_index != loop_exit) {
    ...
    for (int i = PAR - 1; i >= 0; i--) {
      int comp_block_row =
          (BSIZE + block_row - i) &
          (BSIZE - 1);  // read_col > 0 is skipped since it has area overhead
                        // and removing it is harmless
      ...
      if (read_row > 0 && read_col < dim - 1 &&
          read_row < dim - 1)  // read_col > 0 is skipped since it has area
                               // overhead and removing it is harmless
      {
        ref_SR[i][i] = reference[read_index];
      }

      int top =
          (i == PAR - 1) ? last_chunk_col_SR[BSIZE - PAR + 1] : out_SR[i][1];
      int top_left = (comp_col_offset == 0 && i == 0)
                         ? input_v_SR[0]
                         : ((i == 0) ? last_chunk_col_SR[0] : out_SR[i - 1][0]);
      int left = (comp_col_offset == 0 && i == 0)
                     ? input_v_SR[1]
                     : ((i == 0) ? last_chunk_col_SR[1] : out_SR[i - 1][1]);
      ...
      // directly pass input to output if on the first row in the block which is
      // overlapped with the previous block
      ...
      int write_block_row =
          (BSIZE + block_row - (PAR - 1)) &
          (BSIZE - 1);  // write to memory is always PAR - 1 rows behind compute
