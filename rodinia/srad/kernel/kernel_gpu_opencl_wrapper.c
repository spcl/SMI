...
void kernel_gpu_opencl_wrapper(
    ...
    int *iN, int *iS, int *jE, int *jW, int mem_size_i, int mem_size_j,
    int version,
    double *extractTime,  // For image compression kernel (before compute loop)
    double *computeTime,  // For the compute loop, similar to the CUDA version
                          // of the benchmark
  ...
  // Select the first device (previously selected for the context) (if there are
  // multiple devices, choose the first one)
  cl_device_id device;
  device = devices[0];

  // Get the name of the selected device (previously selected for the context)
  // and print it
  ...
  char *kernel_file_path =
      getVersionedKernelName("./kernel/srad_kernel", version);
  ...
  cl_program program = clCreateProgramWithSource(
      context, 1, (const char **)&source, &sourceSize, &error);
#else
  cl_program program =
  ...
  cl_kernel compute_kernel = NULL, prepare_kernel = NULL, reduce_kernel = NULL,
            srad_kernel = NULL, srad2_kernel = NULL;
  ...
  cl_mem d_I;
#ifdef NO_INTERLEAVE
  d_I = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_CHANNEL_1_INTELFPGA,
                       mem_size, NULL, &error);
#else
...
#ifdef NO_INTERLEAVE
    d_I_out =
        clCreateBuffer(context, CL_MEM_READ_WRITE | CL_CHANNEL_2_INTELFPGA,
                       mem_size, NULL, &error);
#else
    d_I_out =
        clCreateBuffer(context, CL_MEM_READ_WRITE, mem_size, NULL, &error);
...
#pragma omp parallel for
  for (int i = 0; i < Ne;
       i++) {  // do for the number of elements in input IMAGE
    image[i] = exp(image[i] /
  ...
  if (is_ndrange_kernel(version))
    printf(
        "max # of workgroups = %d, # of threads/workgroup = %d (ensure that "
        "device can handle)\n",
  ...
  if (version == 5) {
    // Reduction loop exit condition, must be a multiple of reduction unroll
    // factor
    int red_exit =
        (Ne % RED_UNROLL == 0) ? (Ne / RED_UNROLL) : (Ne / RED_UNROLL) + 1;

    // Compute loop exit condition, must be a multiple of comp_bsize
    int comp_bsize = BSIZE - 2 * HALO;
    int last_row =
        (Nr % comp_bsize == 0) ? Nr : Nr + comp_bsize - (Nr % comp_bsize);
    int num_blocks = last_row / comp_bsize;
    int comp_exit = BSIZE * num_blocks * (Nc + 1) / SSIZE;

    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 0, sizeof(fp), (void *)&lambda));
    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 1, sizeof(cl_int), (void *)&Nr));
    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 2, sizeof(cl_int), (void *)&Nc));
    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 3, sizeof(cl_long), (void *)&Ne));
    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 4, sizeof(cl_int), (void *)&red_exit));
    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 5, sizeof(cl_int), (void *)&comp_exit));
    CL_SAFE_CALL(
        clSetKernelArg(compute_kernel, 6, sizeof(cl_int), (void *)&last_row));
  } else {
    //====================================================================================================100
    //	Prepare Kernel
    //====================================================================================================100
    ...
    CL_SAFE_CALL(
        clSetKernelArg(prepare_kernel, 0, sizeof(cl_long), (void *)&Ne));
    CL_SAFE_CALL(
        clSetKernelArg(prepare_kernel, 1, sizeof(cl_mem), (void *)&d_I));
    CL_SAFE_CALL(
        clSetKernelArg(prepare_kernel, 2, sizeof(cl_mem), (void *)&d_sums));
    CL_SAFE_CALL(
        clSetKernelArg(prepare_kernel, 3, sizeof(cl_mem), (void *)&d_sums2));
    ...
    if (is_ndrange_kernel(version)) {
      CL_SAFE_CALL(
          clSetKernelArg(reduce_kernel, 2, sizeof(cl_mem), (void *)&d_sums));
      CL_SAFE_CALL(
          clSetKernelArg(reduce_kernel, 3, sizeof(cl_mem), (void *)&d_sums2));
    } else {
      CL_SAFE_CALL(
          clSetKernelArg(reduce_kernel, 0, sizeof(cl_long), (void *)&Ne));
      CL_SAFE_CALL(
          clSetKernelArg(reduce_kernel, 1, sizeof(cl_mem), (void *)&d_sums));
      CL_SAFE_CALL(
          clSetKernelArg(reduce_kernel, 2, sizeof(cl_mem), (void *)&d_sums2));
    }
    if (version == 3) {
      int loop_exit =
          (Ne % RED_UNROLL == 0) ? (Ne / RED_UNROLL) : (Ne / RED_UNROLL) + 1;
      CL_SAFE_CALL(
          clSetKernelArg(reduce_kernel, 3, sizeof(cl_int), (void *)&loop_exit));
    }
    ...
    if (is_ndrange_kernel(version)) {
      ...
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 1, sizeof(cl_long), (void *)&Ne));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 2, sizeof(cl_mem), (void *)&d_iN));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 3, sizeof(cl_mem), (void *)&d_iS));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 4, sizeof(cl_mem), (void *)&d_jE));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 5, sizeof(cl_mem), (void *)&d_jW));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 6, sizeof(cl_mem), (void *)&d_dN));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 7, sizeof(cl_mem), (void *)&d_dS));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 8, sizeof(cl_mem), (void *)&d_dW));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 9, sizeof(cl_mem), (void *)&d_dE));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 11, sizeof(cl_mem), (void *)&d_c));
      CL_SAFE_CALL(
          clSetKernelArg(srad_kernel, 12, sizeof(cl_mem), (void *)&d_I));
    } else {
      CL_SAFE_CALL(clSetKernelArg(srad_kernel, 0, sizeof(cl_int), (void *)&Nr));
    ...
    if (is_ndrange_kernel(version)) {
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 0, sizeof(fp), (void *)&lambda));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 1, sizeof(cl_int), (void *)&Nr));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 2, sizeof(cl_long), (void *)&Ne));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 3, sizeof(cl_mem), (void *)&d_iS));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 4, sizeof(cl_mem), (void *)&d_jE));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 5, sizeof(cl_mem), (void *)&d_dN));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 6, sizeof(cl_mem), (void *)&d_dS));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 7, sizeof(cl_mem), (void *)&d_dW));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 8, sizeof(cl_mem), (void *)&d_dE));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 9, sizeof(cl_mem), (void *)&d_c));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 10, sizeof(cl_mem), (void *)&d_I));
    } else {
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 0, sizeof(fp), (void *)&lambda));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 1, sizeof(cl_int), (void *)&Nr));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 2, sizeof(cl_int), (void *)&Nc));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 3, sizeof(cl_mem), (void *)&d_iS));
      CL_SAFE_CALL(
          clSetKernelArg(srad2_kernel, 4, sizeof(cl_mem), (void *)&d_jE));
  ...
  // for swapping buffers in v5, deliberately reversed since buffers are swapped
       ...
       iter++)  // do for the number of iterations input parameter
  {
    ...
    if (version == 5) {
      ...
      CL_SAFE_CALL(
          clSetKernelArg(compute_kernel, 7, sizeof(cl_mem), (void *)&src));
      CL_SAFE_CALL(
          clSetKernelArg(compute_kernel, 8, sizeof(cl_mem), (void *)&dst));

      error = clEnqueueTask(command_queue, compute_kernel, 0, NULL, NULL);
      if (error != CL_SUCCESS) fatal_CL(error, __LINE__);

      // wait for kernel to finish before going to next iterations where
      // buffers are swapped
      ...
      if (is_ndrange_kernel(version)) {
        ...
        while (blocks2_work_size != 0) {
          ...
          CL_SAFE_CALL(
              clSetKernelArg(reduce_kernel, 0, sizeof(cl_long), (void *)&no));
          CL_SAFE_CALL(
              clSetKernelArg(reduce_kernel, 1, sizeof(cl_int), (void *)&mul));
          CL_SAFE_CALL(clSetKernelArg(reduce_kernel, 4, sizeof(cl_int),
          ...
          if (blocks2_work_size == 1) {
            ...
            if (blocks2_work_size % (int)local_work_size[0] !=
                0)  // compensate for division remainder above by adding one
                    // grid
            {
              blocks_x = blocks_x + 1;
            }
  ...
  error = clEnqueueReadBuffer(command_queue, output_source, CL_TRUE, 0,
                              mem_size, image, 0, NULL, NULL);
  if (error != CL_SUCCESS) fatal_CL(error, __LINE__);
...
#pragma omp parallel for
  for (int i = 0; i < Ne; i++) {     // do for the number of elements in IMAGE
