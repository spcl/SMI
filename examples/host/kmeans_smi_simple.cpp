...
constexpr auto kUsage =
    "Usage: ./kmeans_smi_simple <[emulator/hardware]> <num_points> <iterations>\n";
...
int main(int argc, char **argv) {
  ...
  if (mode_str == "emulator") {
    ...
    kernel_path =
        ("kmeans_smi_simple_emulator_" + std::to_string(mpi_rank) + ".aocx");
  } else if (mode_str == "hardware") {
    kernel_path = "kmeans_smi_simple_hardware.aocx";
    emulator = false;
  } else {
    std::cout << kUsage;
    return 2;
  ...
  for (int i = 0; i < kChannelsPerRank; ++i) {
    LoadRoutingTable<char>(mpi_rank, i, kNumTags, "kmeans_smi_simple_routing",
                           "ckr", &routing_tables_ckr[i][0]);
    LoadRoutingTable<char>(mpi_rank, i, mpi_size, "kmeans_smi_simple_routing",
  ...
  try {
    ...
    MPIStatus(mpi_rank, "Creating compute kernel...\n");
    std::vector<hlslib::ocl::Kernel> kernels;
    kernels.emplace_back(program.MakeKernel("KMeans", centroids_device,
                                            points_device, points_per_rank,
                                            iterations, mpi_rank, mpi_size));

    // Wait for compute kernels to be initialized
    MPI_Barrier(MPI_COMM_WORLD);

    // Execute kernel
    MPIStatus(mpi_rank, "Launching compute kernel...\n");
