...
channel VTYPE dims_ch __attribute__((depth(8 * DIMS / W)));
channel ITYPE index_ch __attribute__((depth(8)));

kernel void ComputeDistance(__global volatile const VTYPE centroids_global[],
                            __global volatile const VTYPE points[],
...
kernel void ComputeMeans(__global volatile VTYPE centroids_global[],
                         const int num_points, const int smi_rank,
                         const int smi_size) {
  ...
  #pragma unroll 1
  for (int k = 0; k < K; ++k) {
    float divisor = 1.0 / count_updated[k];
    #pragma unroll 1
    for (int d = 0; d < DIMS / W; ++d) {
      VTYPE updated = centroids_updated[d][k] * divisor; 
