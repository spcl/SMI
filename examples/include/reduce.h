...
void SMI_Reduce_float(SMI_RChannel *chan, volatile void* data_snd, volatile void* data_rcv)
{
...
void SMI_Reduce_int(SMI_RChannel *chan, volatile void* data_snd, volatile void* data_rcv)
{
    ...
    SET_HEADER_NUM_ELEMS(chan->net.header,1);
        //offload to reduce kernel
    if(chan->my_rank==chan->root_rank) //root
        write_channel_intel(channel_reduce_send[1],chan->net);
    else //non root
        write_channel_intel(channel_reduce_send_no_root[1],chan->net);
    chan->packet_element_id=0;

    if(chan->my_rank==chan->root_rank)//I'm the root, I have to receive from the kernel
    {
        chan->net_2=read_channel_intel(channel_reduce_recv[1]);
...
__kernel void kernel_reduce_float(const char num_rank)
{
...
//In this implementation we don't need shift register to support II=1
__kernel void kernel_reduce_int(const char num_rank)
{
    SMI_Network_message mess,mess_no_root;
    SMI_Network_message reduce;
    bool init=true;
    ...
    const char credits_flow_control=8; //apparently, this combination (credits, max ranks) is the max that we can support with II=1
    const char max_num_ranks=16;
    int __attribute__((register)) reduce_result[credits_flow_control];  //reduced results
    char data_recvd[credits_flow_control];
    bool send_credits=false;//true if (the root) has to send reduce request
    char credits=credits_flow_control; //the number of credits that I have
    char send_to=0;
    ...
    while(true)
    {
        ...
        if(!send_credits)
        {
            ...
            {   //for the root, I have to receive from both sides
                case 0:
                    mess=read_channel_nb_intel(channel_reduce_send[1],&valid);
                    break;
                case 1: //read from CK_R, can be done by the root and by the non-root
                    mess=read_channel_nb_intel(channels_from_ck_r[/*chan_idx*/2],&valid);
                    break;
            }
            if(valid)
            {
                ...
                if(sender_id==0)
                {
                    ...
                    char * ptr=mess.data;
                    int data= *(int*)(ptr);
                   // printf("Reduce kernel received from app, root. Adding it to: %d (%d + %d)\n",add_to_root,reduce_result[add_to_root],data);
                ...
                else
                {
                    ...
                    {
                        //this is a credit! I can take the message from the app and send it
                        mess_no_root=read_channel_intel(channel_reduce_send_no_root[1]);
                        SET_HEADER_TAG(mess_no_root.header,2); //TODO:fix this
                        write_channel_intel(channels_to_ck_s[3],mess_no_root);
                    }
                    else
                    {
                        //I'm the root
                        ...
                        int data= *(int*)(ptr);
                        char addto=add_to[rank];
                        //printf("Reduce kernel, received from %d: %d (%d+%d)\n",rank,data,reduce_result[addto],data);
                        data_recvd[addto]++;
                        a=addto;
                        reduce_result[addto]+=data;        //SMI_ADD
                        addto++;
                ...
                {   //This could happens only in the root
                    ...
                    for(int jj=0;jj<4;jj++) //copy the data
                        data_snd[jj]=conv[jj];
                    write_channel_intel(channel_reduce_recv[1],reduce);
                    send_credits=true;
                    credits++;
                    data_recvd[current_buffer_element]=0;
                    reduce_result[current_buffer_element]=0;
        ...
        else
        {
            ...
            {
                SET_HEADER_OP(reduce.header,SMI_REQUEST);
                SET_HEADER_TAG(reduce.header,2); //TODO: Fix this tag
                SET_HEADER_DST(reduce.header,send_to);
                write_channel_intel(channels_to_ck_s[4],reduce);
                //printf("Redcue int, sent credit to: %d\n",send_to);
