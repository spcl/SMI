...
def get_channel(graph, key, index) -> Union[Channel, None]:
    for channel in graph.nodes:
        if channel.fpga_key() == key and channel.index == index:
            return channel
    return None


def test_ckr_table():
    graph, paths, ranks = init_graph("""
    N0:F0:ch0 - N0:F1:ch0
    N1:F0:ch0 - N0:F0:ch1
""")
    a = get_channel(graph, "N0:F0", 0)
    assert ckr_routing_table(paths, ranks, a) == [1, 0, 2]

    b = get_channel(graph, "N0:F0", 1)
    assert ckr_routing_table(paths, ranks, b) == [1, 4, 0]

    c = get_channel(graph, "N0:F1", 0)
    assert ckr_routing_table(paths, ranks, c) == [0, 1, 0]

    d = get_channel(graph, "N1:F0", 0)
    assert ckr_routing_table(paths, ranks, d) == [0, 0, 1]
...
""")
    ...
    with pytest.raises(NoRouteFound):
        ckr_routing_table(paths, ranks, ch)
