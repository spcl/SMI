...
__constant char internal_sender_rt[4] = { 0, 1, 2, 3 };
__constant char internal_receiver_rt[4] = { 0, 1, 2, 3 };


//#define QSFP_COUNT 4

channel SMI_Network_message channels_to_ck_s[QSFP_COUNT] __attribute__((depth(16)));
channel SMI_Network_message channels_from_ck_r[QSFP_COUNT] __attribute__((depth(16)));
...
__kernel void CK_S_0(__global volatile char *restrict rt)
{
    ...
    const char num_sender = QSFP_COUNT-1 + 1 +1;
    char sender_id = 0;
...
{
    char external_routing_table[4 /* tag count */];
    for (int i = 0; i < 4 /* tag count */; i++)
    {
        external_routing_table[i] = rt[i];
    }
...
__kernel void CK_S_1(__global volatile char *restrict rt)
{
    ...
    // number of CK_S - 1 + CK_R + 1 tags
    const char num_sender = QSFP_COUNT -1 + 2;
    char sender_id = 0;
    SMI_Network_message message;
