...
int main(int argc, char *argv[])
{
    ...
    cout << "Performing pipeline with  "<<n<<" elements."<<endl;
    int rank_count;
    ...
    {
        if(emulator)
            program_path = ("emulator_" + std::to_string(rank) +"/pipeline.aocx");
        else
            program_path = "pipeline/pipeline.aocx";
    ...
    SMI_Comm comm=SmiInit_pipeline(rank, rank_count, ROUTING_DIR, context, program, buffers);
