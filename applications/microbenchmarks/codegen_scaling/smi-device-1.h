...
__constant char cks_data_table[2] = { -1, -1 };
channel SMI_Network_message cks_data_channels[0] __attribute__((depth(16)));

// cks_control: logical port -> index in cks_control_table -> index in cks_control_channels
__constant char cks_control_table[2] = { 0, 1 };
channel SMI_Network_message cks_control_channels[2] __attribute__((depth(16)));

// ckr_data: logical port -> index in ckr_data_table -> index in ckr_data_channels
__constant char ckr_data_table[2] = { 0, 1 };
channel SMI_Network_message ckr_data_channels[2] __attribute__((depth(BUFFER_SIZE)));

// ckr_control: logical port -> index in ckr_control_table -> index in ckr_control_channels
__constant char ckr_control_table[2] = { -1, -1 };
channel SMI_Network_message ckr_control_channels[0] __attribute__((depth(BUFFER_SIZE)));
