...
//NEW: the  buffer size is a constant choosen by the user
//For the moment being, both FIFO buffers for data and control are defined
//with the same buffer size. We can think of separating them (then we need to properly code generate)
...
//NEW: the maximum number of consecutive reads that each CKs/CKr can do from the same channel
//(there are also small changes in cks/ckr)
...
{
    //NEW, instead of having a fixed rank count, we pass it as parameter
    //MAX_RANKS is the maximum number. It is written in this strange way because it
    //generates less resource demanding design
    ...
    while(1)
    {
        ...
        if(!valid || contiguos_reads==READS_LIMIT) //NEW, keeps reading from the same channel
        {
            contiguos_reads=0;
            sender_id++;
...
__kernel void CK_R_0(__global volatile char *restrict rt, const char rank)
{
    ...
    //NEW: for each tag, we have to consider the control and data port
        ...
        if (valid)
        {
            ...
            switch (dest)
            {
                ...
                case 4:
                    ...
                    write_channel_intel(channels_from_ck_r[internal_from_ckr_control_rt[0]], message); //NEW: generate connection properly for data and control
                    break;
            }
        }
        ...
        if (valid)
        {
            ...
            switch (dest)
            {
                ...
                case 4:
                    ...
                    write_channel_intel(channels_from_ck_r[internal_from_ckr_control_rt[1]], message);
                    break;
