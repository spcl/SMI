...
#define CHANNEL_VECTOR_OUT_A channel_vect_out_A
...
channel TYPE_T CHANNEL_VECTOR_OUT_A __attribute__((depth(W)));
channel TYPE_T CHANNEL_VECTOR_OUT_B __attribute__((depth(W)));
    ...
    for(int ti=0;ti<BlocksY;ti++)
    {
        ...
        for(int tj=0;tj<BlocksX;tj++)
        {
            ...
            for(int i=0;i<tile_y;i++)
            {
                ...
                if(tj==BlocksX-1)
                   write_channel_intel(CHANNEL_VECTOR_OUT_A,result);
            }
        }
    }
}
...
__kernel void axpy(const TYPE_T alpha, int N)
{
    ...
    for(int i=0;i<N;i++)
    {
        float res=read_channel_intel(CHANNEL_VECTOR_OUT_A) + read_channel_intel(CHANNEL_VECTOR_OUT_B);
        write_channel_intel(CHANNEL_VECTOR_OUT,res);
    }
...
__kernel void read_vector_y_A(__global volatile TYPE_T *restrict data, unsigned int N, unsigned int pad_size, unsigned int repetitions)
{
    ...
    for(int t=0; t< repetitions;t++)
    {
        ...
        for(int i=0;i<outer_loop_limit;i++)
        {
            ...
            #pragma unroll
            for(int k=0;k<W;k++)
                write_channel_intel(CHANNEL_VECTOR_Y_A,y[k]);
        }
    }
}
